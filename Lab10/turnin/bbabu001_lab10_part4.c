...
enum FreqStates{adjust1, adjust2}FreqState;
void FreqTick() {
	unsigned char fup = ~PINA & 0x01;
	unsigned char fdown = ~PINA & 0x02;

	switch(FreqState) {
		case adjust1:
			if (fdown == 0x02 && freq < 5) {
				if (check == 1) {
					freq++;
					check = 0;
					FreqState = adjust2;
